...
local fov = Config.Telescope.Zoom.Max
...
local function DisplayNotification(msg)
	QBCore.Functions.Notify(msg, 'error', 1500)
end
...
local function SetupInstructions()
	...
	AddTextComponentSubstringPlayerName(Config.Telescope.Localization.Exit)
...
local function IsPedPlayingAnyTelescopeAnim(ped)
    for animType, animations in pairs(Config.Telescope.Animations) do 
...
local function HandleZoom()
	if GetDisabledControlNormal(0, 32) ~= 0.0 or GetDisabledControlNormal(0, 335) ~= 0.0 then -- Zoom in
        fov = math.max(fov - Config.Telescope.Zoom.Speed, Config.Telescope.Zoom.Min)
    end
    if GetDisabledControlNormal(0, 33) ~= 0.0 or GetDisabledControlNormal(0, 336) ~= 0.0 then -- Zoom out
        fov = math.min(fov + Config.Telescope.Zoom.Speed, Config.Telescope.Zoom.Max)
    end
...
local function HandleMovementInput()
	...
	if axisX ~= 0.0 or axisY ~= 0.0 then
        local zoomValue = (1.0/(Config.Telescope.Zoom.Max-Config.Telescope.Zoom.Min))*(fov-Config.Telescope.Zoom.Min)
        local rotation = GetCamRot(camera, 2)

        local movementSpeed = (IsUsingKeyboard(1) and Config.Telescope.MovementSpeed.Keyboard) or Config.Telescope.MovementSpeed.Controller
...
local function GetClosestTelescope()
    local closest = 0
    local closestDist = Config.Telescope.MaxDetectionDist
    local coords = GetEntityCoords(GetPlayerPed(-1))

    for model, data in pairs(Config.Telescope.Models) do
        local entity = GetClosestObjectOfType(coords.x, coords.y, coords.z, Config.Telescope.MaxDetectionDist, model, false, false, false)
        if entity ~= 0 then
...
local function UseTelescope(entity)
    local playerPed = GetPlayerPed(-1)
    local data = Config.Telescope.Models[GetEntityModel(entity)]
    local offsetCoords = GetOffsetFromEntityInWorldCoords(entity, data.offset)
    local animation = Config.Telescope.Animations[data.animation]
    inTelescope = true

    if not IsTelescopeAvailable(offsetCoords) then
        DisplayNotification(Config.Telescope.Localization.TelescopeInUse)
        return
    end
    ...
    elseif dist > 2.0 then
        DisplayNotification(Config.Telescope.Localization.ToFarAway)
        ClearPedTasks(playerPed)
        return
    end
...
do
    ...
    exports['qtarget']:AddTargetModel(models, {
        options = {
            {
                icon = Config.Telescope.Targeting.Icon,
                label = Config.Telescope.Targeting.Label,
                action = function(entity)
                    UseTelescope(entity)
                end
            },
        },
        distance = Config.Telescope.MaxInteractionDist
    })
end

-- Help Text Thread --
if Config.Telescope.UseDistanceThread then
    CreateThread(function()
        while true do
            ...
            if not inTelescope then
                for index, data in pairs(Config.Telescope.Telescopes) do
                ...
                if closest ~= 0 then
                    if distance < Config.Telescope.MaxInteractionDist then
                        sleep = 0
                        DisplayHelpText(Config.Telescope.Localization.HelpText)
                        if IsControlJustPressed(0, 38) then
                            local telescope = GetClosestTelescope()
                            if telescope ~= 0 then
                                UseTelescope(telescope)
                            else
                                DisplayNotification(Config.Telescope.Localization.NonFound)
                            end
                        end
                    else
                        sleep = distance*20
